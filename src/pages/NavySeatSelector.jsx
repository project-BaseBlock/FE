...
import styles from "../styles/NavySeatSelector.module.css";
import { useSearchParams, useNavigate } from "react-router-dom";

export default function NavySeatSelector() {
  ...
  const zoneName = searchParams.get("zoneName") || "네이비";
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.top}>
        <h2 className={styles.title}>좌석 선택 (네이비 구역)</h2>
        ...
        <div className={styles.left}>
          <div className={styles.seatGrid}>
            {seatRows.map((row, rIdx) => (
              <div key={`N-${rIdx}`} className={styles.seatRow}>
                {row.map((seat) => (
                  <div
                    key={seat}
                    className={[
