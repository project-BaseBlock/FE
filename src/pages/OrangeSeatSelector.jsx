// src/pages/OrangeSeatSelector.jsx
import React, { useEffect, useMemo, useState } from "react";
import axios from "../api/axiosInstance";
import styles from "../styles/OrangeSeatSelector.module.css";
import { useSearchParams, useNavigate } from "react-router-dom";

export default function OrangeSeatSelector() {
  ...
  const zoneName = searchParams.get("zoneName") || "오렌지";
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.top}>
        <h2 className={styles.title}>좌석 선택 (오렌지 구역)</h2>
        ...
        <div className={styles.left}>
          <div className={styles.seatGrid}>
            {seatRows.map((row, rIdx) => (
              <div key={`O-${rIdx}`} className={styles.seatRow}>
                {row.map((seat) => (
                  <div
                    key={seat}
                    className={[
